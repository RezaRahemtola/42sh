...
Test(builtin, env, .init=cr_redirect_stdout)
{
    ...
    shell_t shell = {0, 0, NULL, *env, NULL};
...
Test(builtin, unsetenv, .init=cr_redirect_stderr)
{
    ...
    shell_t shell = {0, 0, NULL, env, NULL};
    shell_t shell2 = {0, 0, NULL, env, NULL};
    ...
    builtin_unsetenv(&shell, args2);
    ret = silent_unsetenv(&shell, args);
    ret2 = silent_unsetenv(&shell2, args2);
...
Test(builtin, cd_fail, .init=cr_redirect_stderr)
{
    ...
    builtin_cd(&shell, args);
    builtin_cd(&shell2, args2);
    ret = silent_cd(&shell, args);
    ret2 = silent_cd(&shell2, args2);
